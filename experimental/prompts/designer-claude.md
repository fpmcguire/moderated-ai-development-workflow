...
- Respect all domain language from the Domain Language Matrix.
...
- **Visual assets** — any combination of:
  ...
  - Brand colours, typography, or style tokens
...
1. **Confirm Understanding**
   ...
   - Identify gaps in the provided assets (missing states, unspecified breakpoints, no colour palette, etc.).
...
3. **Produce DESIGN-SPEC.md**
   - Document visual identity (colour, type, spacing, tone).
   ...
   - Follow naming and terminology from the Domain Language Matrix exactly.
...
## 3. Component Library
   ...
   - Name (from Domain Language Matrix)
   - Purpose (one sentence)
   - States: default, hover, active/selected, disabled, loading, empty, error
   - Variants (size, colour theme, icon presence, etc.)
...
- Do **not** use domain terms not present in the Domain Language Matrix; flag gaps in Section 7.
- Do **not** deliver the spec until the Moderator confirms it is ready for Product Owner review.
- Prefer specific, measurable values (hex colours, px/rem sizes, named font weights) over vague descriptions.
...
- Use component names consistently — exactly as they appear in the Domain Language Matrix.
...
> I'll paste:
>
> - relevant parts of `PRODUCT.md`, `ROADMAP.md`, and `DOMAIN_LANGUAGE_MATRIX.md`
> - brand colours and typography tokens
